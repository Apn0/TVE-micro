...
- On fresh deployments that do not yet have a checked-out branch, the script now falls back to the remote's default branch after fetching. If your current branch is missing on the remote, it will prompt (when interactive) or automatically fall back to the remote default branch unless you set `BRANCH` explicitly. The script also detects the remote default branch directly via `git ls-remote --symref` when the local remote HEAD alias has not been set, and falls back to common default branch names like `main` or `master` when the remote default cannot be detected.
...
- Set `REMOTE` or `BRANCH` environment variables to override the default remote/branch. If no `origin` exists and exactly one remote is configured, the script will use that remote automatically. When a remote is missing, you can supply `REMOTE_URL`, run interactively to choose among existing remotes, accept a prompt to add the URL (useful on fresh devices), or continue in local-only mode (skip fetching) by replying `y` when asked or by setting `ALLOW_LOCAL_ONLY=true`.
