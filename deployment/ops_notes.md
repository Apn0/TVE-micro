...
- On fresh deployments that do not yet have a checked-out branch, the script now falls back to the remote's default branch after fetching. If your current branch is missing on the remote, it will prompt (when interactive) or automatically fall back to the remote default branch unless you set `BRANCH` explicitly.
